...
if __name__ == '__main__':
    ...
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.317, patience=5, verbose=True)
     # 新增：严格按照论文描述
    # step_size=4: 每4个epoch调整一次
    # gamma=0.96: 衰减4% (即乘以 0.96)
    ...
    for epoch in range(1, opt.nepoch + 1):
        ...
        if opt.train == 0:
            ...
            logging.info('epoch: %d, lr: %.7f, loss: %.4f, p1: %.2f, p2: %.2f' % (epoch, current_lr, loss, p1, p2))
            print('e: %d, lr: %.7f, loss: %.4f, p1: %.2f, p2: %.2f' % (epoch, current_lr, loss, p1, p2))
