...
class PoolingAttention(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0.1, proj_drop=0.1):
        super().__init__()
        ...
        self.local_pooling_group_1 = [2, 3, 5, 6, 1, 4, 0, 7, 15, 16, 12, 13, 11, 14]
        self.local_pooling_group_3 = [8, 9, 10]
        self.local_pooling_group_2 = [0, 1, 2, 3, 4, 5, 6, 7]

        self.pool_1 = nn.AvgPool1d(kernel_size=2, stride=2)
        self.pool_2 = nn.AvgPool1d(kernel_size=3, stride=3)
    ...
    def forward(self, x):
        ...
        pool_tmp1 = self.pool_1(x[:, self.local_pooling_group_1].transpose(1, 2))
        pool_tmp2 = self.pool_2(x[:, self.local_pooling_group_3].transpose(1, 2))
        pool = torch.cat([pool_tmp1,pool_tmp2],dim=2)
        pools.append(pool)
